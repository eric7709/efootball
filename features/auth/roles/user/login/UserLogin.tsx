...
export function UserLogin() {
  ...
  return (
    <form className="panel grid" onSubmit={handleSubmit} style={{ maxWidth: 460 }}>
      <div>
        <p className="eyebrow" style={{ margin: 0 }}>WELCOME BACK</p>
        <h1 style={{ marginBottom: 8 }}>Sign in</h1>
        <p className="muted" style={{ margin: 0 }}>Sign in to manage your eFootball activity.</p>
      </div>

      <FormField label="Email address" htmlFor="login-email">
        <TextInput id="login-email" type="email" autoComplete="email" required value={form.email} onChange={(event) => setForm((current) => ({ ...current, email: event.target.value }))} />
      </FormField>
      <FormField label="Password" htmlFor="login-password">
        <TextInput id="login-password" type="password" autoComplete="current-password" required value={form.password} onChange={(event) => setForm((current) => ({ ...current, password: event.target.value }))} />
      </FormField>

      {login.isError ? <p role="alert" style={{ color: "var(--danger)", margin: 0 }}>{getErrorMessage(login.error)}</p> : null}
