...
} from "./useCompetitionFilters";

import type { Competition } from "@/features/competition/types.competition";
...
export function CompetitionAdminPage() {
  ...
  const filteredCompetitions = competitions.filter((competition) => {
    ...
    return (
      matchesStatus &&
      searchableCompetition.includes(query.trim().toLowerCase())
    );
  });
  const pageCount = Math.max(
    1,
    Math.ceil(filteredCompetitions.length / pageSize),
  );
  ...
  const columns: AdminTableColumn<Competition>[] = [
      ...
      render: (competition) => (
        <>
          ...
          <small className="muted">
            {competition.season ?? competition.year}
          </small>
        </>
      ),
    },
    {
      ...
      label: "Format",
      render: (competition) => (
        <span className="competition-table-label">
    ...
    },
    {
      ...
      render: (competition) => (
        <span className="competition-table-label">
          <CompetitionPlatformBadge platform={competition.platform} />
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      label: "Actions",
      render: (competition) => (
        <div className="competition-table-actions">
          <button
            type="button"
            className="competition-admin-action edit"
          ...
          <button
            type="button"
            className="competition-admin-action delete"
  ...
  return (
    <>
      <AdminPageHeader
        ...
        title="Competitions"
        description="Manage tournament formats, registrations, and schedules."
      />
        ...
        <div className="admin-table-toolbar">
          <div className="admin-filter-field admin-search-field">
            ...
            <input
              ...
              type="search"
              placeholder="Name, season, format, or platform"
              value={query}
              onChange={(event) => {
          ...
          <div className="admin-filter-field">
            <label htmlFor="competition-status-filter">Status</label>
            <select
            ...
            >
              ...
              {competitionStatusOptions.map((status) => (
