...
export function TeamsAdminPage() {
  ...
  const { query, page, setQuery, setPage } = useTeamFilters();
  const { openCreateModal, openUpdateModal, openDeleteModal } = useTeamStore();
  const filteredTeams = teams.filter((team) =>
    [team.name, team.tag, team.country, team.region]
      .filter(Boolean)
      .join(" ")
      .toLowerCase()
  ...
  const columns: AdminTableColumn<Team>[] = [
    {
      ...
      render: (team) => (
        <>
          ...
          <br />
          <small className="muted">{team.tag}</small>
        </>
      ),
    },
    {
      ...
      label: "Country",
      render: (team) =>
        team.region ? `${team.country} · ${team.region}` : team.country,
    },
    {
      key: "strength",
      label: "Strength",
      render: (team) => team.strength,
    },
    {
      key: "actions",
      label: "Actions",
  ...
  return (
    <>
        ...
        <div className="admin-table-toolbar">
          <div className="admin-filter-field">
            ...
            <input
              id="team-search"
              type="search"
              placeholder="Name, tag, country, or region"
              value={query}
              onChange={(event) => {
      ...
      <CreateTeamModal
        onCreate={(team) => setTeams((current) => [...current, team])}
      />
      <UpdateTeamModal
        onUpdate={(updated) =>
          setTeams((current) =>
